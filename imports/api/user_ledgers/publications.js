 ...
 */
Meteor.publish('userLedgersByArgs', function userLedgerByArgs(filter, options) {
...
Meteor.publish('UserLedgersBalanceById', function userLedgerBalance() {
...
UserLedgersBalance.recalcAll = () => {
  Meteor.users.find().forEach((u) => {
    let sum = 0;
    UserLedgers.find({ userId: u._id }).forEach((cr) => {
      sum += cr.amount;
    });
